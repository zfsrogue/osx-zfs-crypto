                       ...
                       const char *dsname, void *arg)
{
    ...
    if ((error = dsl_pool_hold(dsname, FTAG, &dp)) != 0)
        return error;
...
zfs_ioc_create(const char *fsname, nvlist_t *innvl, nvlist_t *outnvl)
{
    ...
    printf("looking up crypto_ctx\n");

	if (nvlist_lookup_byte_array(innvl, "crypto_ctx", (char *)&zic,&size) != 0)
		return (EINVAL);

    printf("Pulled out crypto_ctx size %d == %d\n", size, sizeof(zic));

    if ((size != sizeof(*zic)) ||
        (error = zfs_get_crypto_ctx(fsname, zic, &dcc)) != 0) {
    printf("ctx failed %d\n", error);
        return (error);
    }
...
zfs_ioc_clone(const char *fsname, nvlist_t *innvl, nvlist_t *outnvl)
{
	...
	if (nvlist_lookup_byte_array(innvl, "crypto_ctx", &zic, &size) != 0)
		return (EINVAL);
...
static zfs_ioc_vec_t zfs_ioc_vec[] = {
	...
	{ zfs_ioc_crypto_key_unload, NULL, zfs_secpolicy_crypto_keyuse,
	  DATASET_NAME, B_TRUE, POOL_CHECK_SUSPENDED, B_FALSE },
	{ zfs_ioc_crypto_key_inherit, NULL, zfs_secpolicy_crypto_keyuse,
	  DATASET_NAME, B_TRUE, POOL_CHECK_SUSPENDED , B_FALSE},
	{ zfs_ioc_crypto_key_change, NULL, zfs_secpolicy_crypto_keychange,
	  DATASET_NAME, B_TRUE, POOL_CHECK_SUSPENDED | POOL_CHECK_READONLY, B_FALSE },
